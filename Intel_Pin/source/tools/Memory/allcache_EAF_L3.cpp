...
#include "pin.H"
#include "eaf.H"
extern EAF ul3_eaf;              // only the declaration
...
// LAST LEVEL CACHE WITH LRU REPLACEMENT POLICY, and EVICTED ADDRESS FILTER
namespace UL3
{
// 3rd level unified cache: 16 MB, 64 B lines, direct mapped
const UINT32 cacheSize                         = 16 * MEGA;
...
// EAF ul3_eaf( UL3::cacheSize / UL3::lineSize );   // one EAF for the LLC
// Increasing alpha to 16 reduces the number of false positives 24th April 2025
EAF ul3_eaf( UL3::cacheSize / UL3::lineSize , /*alpha=*/16 );   
...
static VOID Fini(int code, VOID* v)
{
    ...
    strftime(filename, sizeof(filename), "EAF_L3-%Y-%m-%d_%H-%M-%S.out", localtime(&now));
...
// Modified L3 to use an Evicted Address Filter
...
static VOID Ul2Access(ADDRINT addr, UINT32 size, CACHE_BASE::ACCESS_TYPE accessType)
{
    ...
    // Replace the plain call ul3.Access() with a dedicated wrapper so we can intercept the miss path:
    if (!ul2Hit) ul3.UL3AccessEAF(addr,size,accessType);
